...
function Login() {
  ...
  const signIn = async (event) => {
    ...
    const email = loginId.includes('@') ? loginId : `${loginId}@workers.sunrega.local`
    const { error: authError } = await supabase.auth.signInWithPassword({ email, password: form.get('password') })
    if (authError) setError(authError.message)
    setBusy(false)
  }
  return <div className="auth-page"><form className="auth-card" onSubmit={signIn}><img src={sunregaLogo} alt="Sunrega" /><div className="auth-heading"><span>TEAM PORTAL</span><h1>Welcome back</h1><p>Sign in to manage leads and solar projects.</p></div>{error && <div className="auth-error">{error}</div>}<label><span>Email or worker ID</span><input name="loginId" type="text" autoComplete="username" required placeholder="admin@sunrega.com or SUN001" /></label><label><span>Password</span><input name="password" type="password" autoComplete="current-password" required placeholder="Enter your password" /></label><button className="primary auth-submit" disabled={busy}>{busy ? 'Signing in…' : 'Sign in securely'}</button><small className="auth-help">Contact your administrator if you need an account.</small></form></div>
}
...
function UserManagement() {
  ...
  const createUser = async (event) => {
    ...
    const form = new FormData(event.currentTarget)
    const { error } = await supabase.functions.invoke('admin-create-user', { body: { userId: form.get('userId'), fullName: form.get('fullName'), password: form.get('password'), phone: form.get('phone'), designation: form.get('designation'), department: form.get('department'), role: form.get('role') } })
    if (error) setMessage(error.message); else { setMessage('User created successfully.'); event.currentTarget.reset(); await loadUsers() }
    setBusy(false)
  }
  return <div className="users-grid"><form className="panel create-user" onSubmit={createUser}><h2>Create team login</h2><p className="muted">Choose the worker's login ID, password and employment details.</p>{message && <div className="user-message">{message}</div>}<label><span>Worker ID</span><input name="userId" required pattern="[A-Za-z0-9._-]+" placeholder="SUN001" /></label><label><span>Full name</span><input name="fullName" required /></label><label><span>Password</span><input name="password" type="password" minLength="8" required /></label><label><span>Phone number</span><input name="phone" type="tel" /></label><label><span>Designation</span><input name="designation" placeholder="Sales Executive" /></label><label><span>Department</span><input name="department" placeholder="Sales" /></label><label><span>Access level</span><select name="role" defaultValue="worker"><option value="worker">Worker</option><option value="admin">Administrator</option></select></label><button className="primary" disabled={busy}>{busy ? 'Creating…' : 'Create user'}</button></form><div className="panel table-wrap"><div className="user-list-title"><h2>Team access</h2><span>{users.length} users</span></div><table><thead><tr><th>Name</th><th>Worker ID</th><th>Designation</th><th>Department</th><th>Role</th><th>Status</th></tr></thead><tbody>{users.map((user) => <tr key={user.id}><td><b>{user.full_name || '—'}</b><small>{user.phone || ''}</small></td><td>{user.user_id || user.email}</td><td>{user.designation || '—'}</td><td>{user.department || '—'}</td><td><Badge text={user.role} /></td><td>{user.is_active ? 'Active' : 'Disabled'}</td></tr>)}</tbody></table></div></div>
}
...
const SPECIFICATIONS = [
  ...
  ['Solar Panels', 'Topcon Bifacial (Premier / Eastman / Satvik)', 'Topcon Bifacial (Waaree / Luminous / Adani / Polycab)'],
...
const defaultQuote = () => ({
  ...
  panel_count: 0, panel_wattage: 620, basic_inverter_count: 1, basic_inverter_kw: 0,
  premium_inverter_count: 1, premium_inverter_kw: 0,
...
function QuotationModal({ item, leads, onSave, onDelete, close }) {
  ...
  const specValue = (name, packageName, fallback) => {
    ...
    if (name === 'Solar Panels') return `${quote.panel_count || 0} × ${quote.panel_wattage || 620} Wp ${fallback}`
    if (name === 'Inverter') return packageName === 'basic'
...
function printQuotation(quote) {
  ...
  const rows = SPECIFICATIONS.map(([name, basic, premium]) => `<tr><th>${htmlEscape(name)}</th><td>${htmlEscape(equipmentValue(name, 'basic', basic))}</td><td>${htmlEscape(equipmentValue(name, 'premium', premium))}</td></tr>`).join('')
